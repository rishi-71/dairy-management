...
export default function AiAssistant() {
  ...
  async function sendMessage(e?: React.FormEvent | React.KeyboardEvent | React.MouseEvent) {
        ...
        <div className="mb-4 w-[350px] sm:w-[420px] h-[600px] bg-slate-50 rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col animate-in slide-in-from-bottom-5 fade-in duration-300">
          ...
          <div className="bg-slate-900 p-4 flex justify-between items-center text-white shadow-md z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-black text-[15px] tracking-widest uppercase">Dairy AI Core</h3>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className={`w-2 h-2 rounded-full ${chat.status === 'streaming' || chat.status === 'submitted' ? 'bg-emerald-400 animate-ping' : 'bg-emerald-500'}`}></span>
            ...
            <button onClick={() => setIsOpen(false)} className="text-slate-400 hover:text-white hover:bg-white/10 transition-colors rounded-full p-2">
          ...
          <div className="flex-1 p-5 overflow-y-auto space-y-5">
            ...
            {chat.messages.map((msg) => (
              <div key={msg.id} className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
                ...
                <div className={`max-w-[90%] rounded-2xl p-4 text-sm shadow-sm ${msg.role === "user" ? "bg-emerald-600 text-white font-medium rounded-br-sm" : "bg-white text-slate-700 border border-slate-200 rounded-tl-sm"}`}>
                  ...
                  {msg.parts?.map((part: any, index: number) => {
                    ...
                    if (part.type === "tool-getCustomerByName") {
                      if (part.state !== "output-available") {
                        return <LoadingTool key={index} text="Searching customer details..." />;
                      }
                      const customer = part.output;
                      if (!customer) return <ErrorTool key={index} text="Customer not found" />;

                      return (
                        <div key={index} className="mt-3 bg-slate-50 border border-emerald-100 rounded-xl p-4 shadow-sm">
                          <h3 className="font-black text-emerald-800 border-b border-emerald-100 pb-2 mb-3 flex items-center gap-2">
                            👤 Customer Details
                          </h3>
                          <div className="space-y-2 text-sm">
                            <p><span className="text-slate-400 font-bold w-20 inline-block">Name:</span> <span className="font-semibold text-slate-800">{customer.name}</span></p>
                            <p><span className="text-slate-400 font-bold w-20 inline-block">Mobile:</span> <span className="font-semibold text-slate-800">{customer.mobile}</span></p>
                            <p><span className="text-slate-400 font-bold w-20 inline-block">Balance:</span> <span className="font-bold text-rose-600">₹{customer.openingBalance}</span></p>
                            <p><span className="text-slate-400 font-bold w-20 inline-block">Address:</span> <span className="text-slate-700">{customer.address}</span></p>
                          </div>
                          
                          {customer.subscriptions?.length > 0 && (
                            <div className="mt-4 pt-3 border-t border-emerald-100">
                              <span className="text-xs font-black text-emerald-600 uppercase tracking-wider mb-2 block">Active Subscriptions</span>
                              <div className="space-y-2">
                                {customer.subscriptions.map((sub: any) => (
                                  <div key={sub.id} className="bg-white border border-slate-200 rounded-lg p-2 flex justify-between items-center shadow-sm">
                                    <span className="font-bold text-slate-700">{sub.itemName}</span>
                                    <div className="flex gap-2 text-xs font-bold">
                                      <span className="bg-amber-100 text-amber-800 px-2 py-1 rounded">M: {sub.morningQty}</span>
                                      <span className="bg-indigo-100 text-indigo-800 px-2 py-1 rounded">E: {sub.eveningQty}</span>
                                    </div>
                                  </div>
                                ))}
                              </div>
                    ...
                    if (part.type === "tool-getCustomersList") {
                      if (part.state !== "output-available") {
                        return <LoadingTool key={index} text="Loading all customers..." />;
                      }
                      return (
                        <div key={index} className="mt-3 bg-white border border-emerald-200 rounded-xl overflow-hidden shadow-sm">
                          <div className="bg-emerald-50 px-4 py-3 border-b border-emerald-100">
                            <h3 className="font-black text-emerald-800 flex items-center gap-2">👥 Customer Directory</h3>
                          </div>
                          <div className="max-h-[250px] overflow-y-auto divide-y divide-slate-100">
                            {part.output?.map((customer: any) => (
                              <div key={customer.id} className="p-3 hover:bg-slate-50 transition-colors flex justify-between items-center">
                                <div>
                                  <div className="font-bold text-slate-800">{customer.name}</div>
                                  <div className="text-xs text-slate-500 font-medium tracking-wide">📞 {customer.mobile}</div>
                                </div>
                    ...
                    if (part.type === "tool-getDashboardStats") {
                      if (part.state !== "output-available") {
                        return <LoadingTool key={index} text="Calculating live statistics..." />;
                      }
                      const stats = part.output;
                      return (
                        <div key={index} className="mt-3 bg-slate-900 rounded-xl p-4 shadow-md text-white">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="bg-slate-800 p-3 rounded-lg border border-slate-700">
                              ...
                              <p className="text-xl font-black text-white">{stats.totalCustomers}</p>
                            </div>
                            <div className="bg-slate-800 p-3 rounded-lg border border-slate-700">
                            ...
                            <div className="bg-slate-800 p-3 rounded-lg border border-slate-700 col-span-2 flex justify-between items-center">
                              <div>
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Morning Req.</p>
                                <p className="text-lg font-black text-amber-400">{stats.morningRequirement} Liters</p>
                              </div>
                              <div className="text-right">
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Total Ledger</p>
                                <p className="text-lg font-black text-emerald-400">₹{stats.outstandingLedger}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    if (part.type === "tool-createCustomer") {
                      if (part.state !== "output-available") {
                        return <LoadingTool key={index} text="Registering new customer..." />;
                      }
                      const customer = part.output;
                      return (
                        <div key={index} className="mt-3 bg-emerald-50 border border-emerald-300 rounded-xl p-4 shadow-sm">
...
function LoadingTool({ text }: { text: string }) {
  return (
    <div className="mt-3 text-xs text-slate-400 font-bold flex items-center gap-2 animate-pulse bg-slate-50 p-3 rounded-lg border border-slate-100">
      <svg className="w-4 h-4 animate-spin text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
      {text}
    </div>
  );
}
