...
export default function LedgerClient({ customer }: { customer: any }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* 🚀 HEADER PANEL */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center rounded-3xl border border-white/60 bg-white/70 p-6 sm:p-8 shadow-lg shadow-emerald-950/5 backdrop-blur-xl gap-4">
      ...
      <div className="rounded-3xl border border-slate-200/60 bg-white shadow-xl overflow-hidden relative">
        <div className="bg-emerald-600 text-white p-3 text-center text-xs font-bold tracking-widest uppercase shadow-md relative z-20">
                ...
                ledgerData.map((day) => {
                  ...
                  return (
                    <tr 
                      ...
                      className={`group cursor-pointer transition-all hover:bg-emerald-50/50 ${hasData ? '' : 'bg-slate-50/30'}`}
                    >
                      <td className="px-6 py-3 whitespace-nowrap">
                        <div className={`flex flex-col items-center justify-center w-12 h-12 rounded-xl border transition-colors ${hasData ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : 'bg-white border-slate-100 text-slate-400 group-hover:bg-emerald-100 group-hover:text-emerald-700 group-hover:border-emerald-200'}`}>
                      ...
                      <td className="px-6 py-4">
                        {!hasData ? <span className="text-xs font-semibold text-slate-300 italic group-hover:text-emerald-600 transition-colors">Tap to add delivery +</span> : (
            ...
            <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
              ...
              <div className="space-y-3">
                <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-wider flex justify-between">
                  ...
                  <span className="text-[9px] text-rose-400 normal-case">(Set both 0 to remove)</span>
                </h4>
                {editLogs.map((log, idx) => (
                  <div key={`log-${log.itemId}`} className="p-3 bg-emerald-50/50 border border-emerald-100 rounded-2xl flex items-center justify-between gap-4 relative group">
                    <div>
                ...
                <select 
                  ...
                  className="w-full text-xs font-bold text-emerald-600 bg-white border border-dashed border-emerald-200 rounded-xl px-3 py-2 outline-none cursor-pointer hover:bg-emerald-50"
                >
              ...
              <div className="space-y-3 pt-4 border-t border-slate-100">
                <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-wider flex justify-between">
                  ...
                  <span className="text-[9px] text-rose-400 normal-case">(Set 0 to remove)</span>
                </h4>
                {editExtras.map((ext, idx) => (
                  <div key={`ext-${ext.itemId}`} className="p-3 bg-amber-50/50 border border-amber-100 rounded-2xl flex items-center justify-between gap-4">
                    <div>
                ...
                <select 
                  ...
                  className="w-full text-xs font-bold text-amber-600 bg-white border border-dashed border-amber-200 rounded-xl px-3 py-2 outline-none cursor-pointer hover:bg-amber-50"
                >
