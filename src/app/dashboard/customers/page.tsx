    ...
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-emerald-50 via-slate-50 to-teal-100 p-4 sm:p-8 text-slate-900">
      ...
      <div className="relative z-10 mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-12">
          ...
          {/* LEFT COLUMN (Stats & Add Form) - Takes up 4/12 columns on large screens */}
          <div className="space-y-6 lg:col-span-4 position-sticky top-8">
            ...
            <div className="rounded-3xl border border-white/60 bg-white/80 p-6 shadow-xl shadow-emerald-900/5 backdrop-blur-xl">
              ...
              <form action={addCustomer} className="space-y-4">
                ...
                <div className="pt-2">
                  <button type="submit" className="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-emerald-500/30 transition-all hover:-translate-y-0.5 hover:bg-emerald-700 hover:shadow-emerald-600/40">
                    + Save Customer
                  </button>
                </div>
              </form>
          ...
          {/* RIGHT COLUMN (Directory Table) - Takes up 8/12 columns on large screens */}
                    ...
                    {customers.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="p-12 text-center text-sm font-medium text-slate-500">
                          No customers found. Use the quick add form to create one!
                        </td>
                      </tr>
                    ) : (
                      customers.map((customer: any) => (
                        <tr key={customer._id} className="transition-colors hover:bg-white/90">
                          ...
                          <td className="whitespace-nowrap px-6 py-5 text-sm flex justify-end gap-4 items-center">
                            <Link href={`/dashboard/customers/${customer._id}`} className="font-semibold text-emerald-600 transition-colors hover:text-emerald-800">
                              Edit
                            </Link>
                            <form action={deleteCustomer}>
                              <input type="hidden" name="id" value={customer._id} />
                              <button type="submit" className="font-semibold text-rose-500 transition-colors hover:text-rose-700">
                                Delete
                              </button>
                            </form>
                          </td>
